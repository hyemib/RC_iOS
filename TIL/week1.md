...
project를 빌드하는 동안 빌드 시스템에서 수행한 작업을 편집하고 재정렬

target의 product를 빌드하기 위해 필요한 파일을 단계 별로 어떻게 처리할 지 구성한 목록

</br>

#### Build Rules

컴파일 단계에서 리소스 복사 단계에서 파일 종류 별로 어떻게 처리할지 규칙 결정

확장자 매칭 조건과 처리할 액션에 대한 지정을 할 수 있는데 기본적인 파일들에 대한 처리 규격은 이미 자동적으로 생성
